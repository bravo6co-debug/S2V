...
function buildPass2Prompt(pass1Result: any, perScene: number, mainCharacters?: any[]): string {
...
${mainCharacters.map((c: any, i: number) => `[${i + 1}] ${c.name} (${c.gender === 'female' ? '여성' : c.gender === 'male' ? '남성' : '미지정'}, ${c.ageGroup})
- 외모: ${c.appearance}
- 복장: ${c.outfit}`).join('\n\n')}
...
function mergeResults(pass1: any, pass2: any, perScene: number) {
  ...
  return {
    ...
    characters: (pass1.mainCharacters || []).map((c: any, idx: number) => {
      // 모든 씬에 등장한다고 가정 (메인 캐릭터)
      const allSceneNumbers = pass1.scenes.map((s: any, i: number) => s.sceneNumber || i + 1);
      return {
        id: crypto.randomUUID(),
